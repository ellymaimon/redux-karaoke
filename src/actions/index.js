...
export function fetchSongId(title) {
  return function (dispatch) {
    ...
    return fetch(`http://api.musixmatch.com/ws/1.1/track.search?&q_track=${title}&page_size=1&s_track_rating=desc&apikey=ae75100d21ead154d0502672ad52890b`).then(
...
export function fetchLyrics(title, artist, musicMatchId, localSongId, dispatch) {
  return fetch(`http://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=${musicMatchId}&apikey=ae75100d21ead154d0502672ad52890b`).then(
    response => response.json(),
    error => console.log('An error occurred.', error)
  ).then(function(json) {
